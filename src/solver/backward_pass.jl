...
function expand_term_L!(
    ...
    tmp.xx_hess = ForwardDiff.hessian!(tmp.xx_hess, params.bwd_cost.terminal, tmp.x)
...
function expand_stage_L!(
    ...
    tmp.xx_hess =
        ForwardDiff.hessian!(tmp.xx_hess, δx -> params.bwd_cost.stage(δx, tmp.u), tmp.x)
    tmp.uu_hess =
        ForwardDiff.hessian!(tmp.uu_hess, δu -> params.bwd_cost.stage(tmp.x, δu), tmp.u)
...
function expand_F!(
    ...
    if typeof(params.bwd_sys) === HybridSystem
        if trn_sym != NULL_TRANSITION
            ...
            ForwardDiff.jacobian!(tmp.xx2, δx -> rk4(δx, u, params.Δt, mode.flow), x)
            mul!(F.x, tmp.xx1, tmp.xx2)

            # Hybrid dynamics jacobian wrt u: Ξ*Fu
            ForwardDiff.jacobian!(tmp.xu, δu -> rk4(x, δu, params.Δt, mode.flow), u)
            mul!(F.u, tmp.xx1, tmp.xu)
        else
            # Get dynamics jacobian wrt x
            ForwardDiff.jacobian!(F.x, δx -> rk4(δx, u, params.Δt, mode.flow), x)
            # Get dynamics jacobian wrt u
            ForwardDiff.jacobian!(F.u, δu -> rk4(x, δu, params.Δt, mode.flow), u)
        end
    else
        # Get dynamics jacobian wrt x
        ForwardDiff.jacobian!(F.x, δx -> rk4(δx, u, params.Δt, params.bwd_sys), x)
        # Get dynamics jacobian wrt u
        ForwardDiff.jacobian!(F.u, δu -> rk4(x, δu, params.Δt, params.bwd_sys), u)
    end
    return
end
...
function update_cost_prediction!(bwd::BackwardCache, fwd::ForwardCache, k::Int)::Nothing
    ...
    bwd.ΔJ2 +=
        (bwd.ds[k]'*Q.uu*bwd.ds[k] + fwd.f̃s[k]'*(2*V.xx*fwd.xs[k] - V.xx*fwd.f̃s[k]))
    return
end
